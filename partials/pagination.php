...
if ( strpos( $posts_pagination, 'prev page-numbers' ) === false ) {
	$pagination_classes = ' only-next bg-color-dark text-white text-decoration-none';
} elseif ( strpos( $posts_pagination, 'next page-numbers' ) === false ) {
	$pagination_classes = ' only-prev bg-color-dark text-white text-decoration-none';
} else {
	$pagination_classes = '';
}
...
if ( $posts_pagination ) { ?>
	...
	<div class="pagination-wrapper --m-auto --max-w-wide --px mt-2 mb-2 subline alignwide <?= ( true === is_home() && false === is_front_page() )?'f-w':'' ?>">
