...
<section id="slick-homeslide" class="mb-0 mb-sm-1 mb-lg-2 contrast--light">
	<div class="container-fluid px-0">
		<div class="row g-0 align-items-center vh-50 md-min-h-460"> <!-- .vh-50 hack--> 
			<div class="col-lg order-1 order-md-last overflow-hidden position-relative" data-aos="fade-right">
					...
					<div class="img-shifted shift-left vh-50 md-min-h-460 <?= $mode; ?>">
						...
						<?php if (!empty($video)): ?>
							<figure class="wp-block-video h-100 bg-action-1">
								<video class="w-100 h-auto video-cover" muted autoplay loop playsinline src="<?= $video['src']; ?>"></video> <!--  style="/* filter: grayscale(100%) brightness(0.6) contrast(1); mix-blend-mode: screen; */" -->
							</figure>
						<?php endif; ?>
						</div>
					</div>
			...
			<div class="col-md-6 col-lg-5 order-2 order-md-first zi-5" data-aos="fade-down">
				...
				<div class="slider-for">
					...
					?>
						...
						<div class="d-flex flex-column justify-content-center justify-content-lg-between bg-color-dark text-white vh-50 md-min-h-460 position-relative rounded-bottom-4 rounded-bottom-right-0 lg-rounded-0" data-post-id="<?= $slide_id; ?>" data-slide-id="<?= $slide_nb; ?>" data-slide-title="<?= $slide_title; ?>" <?= $style; ?>>
							<div class="px-3 px-md-4 px-xl-6 pt-18 d-none d-md-block"><?= (($label != '')?'<h6 class="headline d-inline '.$class.'">'.$label.'</h6>':''); ?></div>
							<div class="px-3 px-md-4 px-xl-6 py-2">
								<h2 class="fw-medium <?= $class ?> p-0"><?= WaffTwo\Core\waff_do_markdown($slide_title); ?></h2>
							</div>
							<div class="px-3 px-md-4 px-xl-6 pb-6 d-none d-md-block">
