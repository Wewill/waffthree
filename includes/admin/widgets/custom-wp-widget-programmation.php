...
?>
			...
			<?php while ( $projections->have_posts() ) : $projections->the_post(); ?>
				...
				// Render HTML 
				print('<div class="modal-body container-fluid" style="position: relative;overflow-x: hidden;overflow-y: scroll;height: 100%;" id="programmationModalToScroll">');
									...
									if ( array_key_exists('has_films', $the_day_rooms) && $the_day_room['has_films'] === true ) {
										...
										foreach($the_day_room['projections'] as $key => $the_day_room_projections) {
											//if ( $the_day_room_projections['has_films'] === true ) {
												...
												if ( $the_day_room_projections['p_highlights'] != '' )		$html_f_tags .= ' <i class="icon icon-sun" data-bs-toggle="tooltip" data-container=".modal-body" title="Temps-fort"></i>'; 
		...
		?>
			...
			<div class="modal-footer container-fluid d-none d-xl-flex">
				<?php if (get_theme_mod( 'planning_url')) : ?><a href="<?php WaffTwo\display_planning_url(); ?>" class="btn btn-link text-decoration-none link-light color-light mx-4"><i class="icon icon-time f-24 lh-48"></i> <span class="subline d-block"><?= esc_html__( 'Planner', 'waff' ) ?> *.pdf</span></a><?php endif; ?>
				<?php if (get_theme_mod( 'booklet_url')) : ?><a href="<?php WaffTwo\display_booklet_url(); ?>" class="btn btn-link text-decoration-none link-light color-light mx-4"><i class="icon icon-brochure f-24 lh-48"></i> <span class="subline d-block"><?= esc_html__( 'Booklet', 'waff' ) ?> *.pdf</span></a><?php endif; ?>
				<?php if (get_theme_mod( 'catalog_url')) : ?><a href="<?php WaffTwo\display_catalog_url(); ?>" class="btn btn-link text-decoration-none link-light color-light mx-4"><i class="icon icon-catalogue f-24 lh-48"></i> <span class="subline d-block"><?= esc_html__( 'Book', 'waff' ) ?></span></a><?php endif; ?>
			</div>
			</div>
		</div>
		</div>
